...
if not errorlevel 1 (
    ...
    echo Python launcher with Python 3 was not found. Install Python 3.10 or newer and run this script again.
    exit /b 1
)
...
if not exist "%ROOT%.venv\Scripts\python.exe" (
    ...
    if errorlevel 1 (
        echo Failed to create %ROOT%.venv. Ensure Python 3 includes venv support.
        exit /b 1
    )
    "%ROOT%.venv\Scripts\python.exe" -m pip install --upgrade pip
    if errorlevel 1 (
        echo Failed to upgrade pip in %ROOT%.venv.
        exit /b 1
    )
    "%ROOT%.venv\Scripts\python.exe" -m pip install -r "%ROOT%auto_rx\requirements.txt"
    if errorlevel 1 (
        echo Failed to install auto_rx requirements in %ROOT%.venv.
        exit /b 1
    )
)

if not exist "%ROOT%.venv\Scripts\python.exe" (
    echo Python virtual environment is incomplete: %ROOT%.venv\Scripts\python.exe is missing.
    exit /b 1
)
